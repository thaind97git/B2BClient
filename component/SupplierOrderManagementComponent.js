...
const getSupplierTable = (orderData = []) => {
  return orderData
    ? orderData.map((order = {}) => ({
        key: order.id,
        ...
        actions: (
          <Button
            onClick={() => {
              Router.push(
                `/supplier/order/details?id=${get('product.id')(order)}`
              );
            }}
            size="small"
            type="link"
...
const SupplierOrderManagementComponent = ({ currentUser }) => {
